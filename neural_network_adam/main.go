...
//function used to take two matrix in memory and put them into single matrix with references to mem.
func Pack(m1 [][]float64, m2 [][]float64) []float64 {
	var result []float64
	for r := 0; r < len(m1); r++ {
		for c := 0; c < len(m1[0]); c++ {
			result = append(result, m1[r][c])
		}
	}
	for r := 0; r < len(m2); r++ {
		for c := 0; c < len(m2[0]); c++ {
			result = append(result, m2[r][c])
		}
	}
	return result
}

//function used to multiply the matrix in place
func MultiplyInPlace(m1 *[]float64, num float64) {
	for r := 0; r < len(*m1); r++ {
		(*m1)[r] *= num
	}
}

//function used to unpack the memory locations into 2 matrices.
func Unpack(m1 *[]float64, vrows int, vcols int, wrows int, wcols int) [5]*float64 {
...
func main() {
	...
	//test pack and unpack here!
	packedValues := Pack(w, v)
	klog.Infof("packedValues= %v\n", packedValues)

	//check unpack
	unpacked := Unpack(&packedValues, 5, 5, 2, 2)
	klog.Infof("unpacked= %v\n", unpacked)
	klog.Infof("unpacked[0]= %v\n", *unpacked[0])
	klog.Infof("unpacked[1]= %v\n", *unpacked[1])

	//check if we can mulitply packedValues by 0.1 without creating new space...
	//we should be able to make mod's to packedValues without having to modify unpacked to view changes.
	//packedValues *= 0.1
	MultiplyInPlace(&packedValues, 0.1)
